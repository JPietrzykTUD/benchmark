...
std::string benchmark_impl(const std::string& function_name, BenchmarkFunction && fun, std::tuple< BenchmarkParams... >&& params, std::tuple< BenchmarkSpecs... >&& specs, FinalizeFunction && fun_finalize = nullptr ) {
  //Sanity checking
  using function_return_t = std::invoke_result_t< BenchmarkFunction, typename BenchmarkParams::type...>;
  static_assert(
      !( std::is_void_v<function_return_t> && std::is_null_pointer_v< FinalizeFunction >),
      "Finalizer Function has to be specified if benchmark function does not return a result."
  );
  if constexpr(!std::is_null_pointer_v< FinalizeFunction >) {
    if constexpr(std::is_void_v<function_return_t>) {
      static_assert(
          (!std::is_void_v<std::invoke_result_t< FinalizeFunction, typename BenchmarkParams::type...>>),
          "Finalizer Function should process the same parameter input as the benchmark function and should return something."
      );
    } else {
      static_assert(
          (!std::is_void_v<std::invoke_result_t< FinalizeFunction, function_return_t, typename BenchmarkParams::type...>>),
          "Finalizer Function should process the result of the benchmark function and the parameter input and should return something"
      );
    }
  }
  ...
  auto dummy = details::benchmark_execute_single_with_pcm<function_return_t, BenchmarkFunction, decltype(function_parameters), FinalizeFunction>(
      std::cout,
  ...
  std::vector<decltype(dummy)> results{};

  for(auto rep = 0; rep < 20; ++rep) {
    auto result =
      details::benchmark_execute_single<function_return_t, BenchmarkFunction, decltype(function_parameters), FinalizeFunction>(
          std::forward<BenchmarkFunction>(fun),
          std::forward<decltype(function_parameters)>(function_parameters),
          std::forward<FinalizeFunction>(fun_finalize)
      );
...
template< typename BenchmarkFunction, typename... BenchmarkParams, typename... BenchmarkSpecs, typename FinalizeFunction = std::nullptr_t >
std::string benchmark_multithread_impl(

    const std::string& function_name,
    BenchmarkFunction && fun,
    std::tuple< BenchmarkParams... >&& params, std::tuple< BenchmarkSpecs... >&& specs,
    FinalizeFunction && fun_finalize = nullptr
) {
  //Sanity checking
  using function_return_t = std::invoke_result_t< BenchmarkFunction, typename BenchmarkParams::type...>;
  static_assert(
